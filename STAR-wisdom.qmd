...
A counterexample is if kind has a value like 5 that doesn’t match any defined class type.

Stabilty

Stability means that the relationship between the columns in the Population Table is the same

A counterexample is if small classes raise scores here but have no effect for students in another district.

Representativeness

Representativeness, or the lack thereof, concerns two relationship, among the rows in the Population Table

A counterexample is using Tennessee-only data to make predictions for Chicago students.


```{r}
#| warning: false
library(tidyverse)
# Basic boxplot: g4math by kind
x %>%
